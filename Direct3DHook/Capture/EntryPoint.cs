...
namespace Capture {
    public class EntryPoint : IEntryPoint {
        ...
        private readonly List<IDXHook> _directXHooks = new List<IDXHook>();
        ...
        public void Run(RemoteHooking.IContext context, string channelName, CaptureConfig config) {
            ...
            currentDomain.AssemblyResolve += (sender, args) => { return GetType().Assembly.FullName == args.Name ? GetType().Assembly : null; };
            ...
            try {
                // Initialise the Hook
                if (!InitialiseDirectXHook(config)) return;
        ...
        /// </summary>
        private void StopCheckHostIsAliveThread() => Interlocked.Increment(ref _stopCheckAlive);
