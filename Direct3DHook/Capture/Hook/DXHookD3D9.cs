...
namespace Capture.Hook {
    internal class DXHookD3D9 : BaseDXHook {
        ...
        // Used in the overlay
        private unsafe int PresentExHook(IntPtr devicePtr,
                ...
                if (Request != null) {
                    ...
                    try {
                        using (var renderTarget = device.GetRenderTarget(0)) {
                            ...
                            if (_renderTargetCopy != null && (_renderTargetCopy.Description.Width != width || _renderTargetCopy.Description.Height != height ||
                            ...
                            if (!_resourcesInitialised || _renderTargetCopy == null) CreateResources(device, width, height, renderTarget.Description.Format);
        ...
        [UnmanagedFunctionPointer(CallingConvention.StdCall, CharSet = CharSet.Unicode, SetLastError = true)]
        private unsafe delegate int Direct3D9Device_PresentDelegate(IntPtr devicePtr, Rectangle* pSourceRect, Rectangle* pDestRect, IntPtr hDestWindowOverride, IntPtr pDirtyRegion);

        [UnmanagedFunctionPointer(CallingConvention.StdCall, CharSet = CharSet.Unicode, SetLastError = true)]
        private unsafe delegate int Direct3D9DeviceEx_PresentExDelegate(IntPtr devicePtr,
                                                                        Rectangle* pSourceRect,
                                                                        Rectangle* pDestRect,
                                                                        IntPtr hDestWindowOverride,
                                                                        IntPtr pDirtyRegion,
