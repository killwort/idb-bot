...
namespace Capture.Hook {
    internal abstract class BaseDXHook : Component, IDXHook {
        ...
        protected int ProcessId {
            get {
                if (_processId == 0) _processId = RemoteHooking.GetCurrentProcessId();
                return _processId;
            }
        }
        ...
        ~BaseDXHook() => Dispose(false);

        protected virtual void InterfaceEventProxy_ScreenshotRequested(ScreenshotRequest request) => Request = request;
        ...
        protected IntPtr[] GetVTblAddresses(IntPtr pointer, int numberOfMethods) => GetVTblAddresses(pointer, 0, numberOfMethods);
        ...
        protected void ProcessCapture(int width,
                                      int height,
                                      int pitch,
                                      PixelFormat format,
                                      IntPtr pBits,
        ...
        protected void ProcessCapture(byte[] bitmapData, ScreenshotRequest request) {
            try {
                if (request != null)
                    Interface.SendScreenshotResponse(new Screenshot(request.RequestId, bitmapData) {Format = request.Format});
                LastCaptureTime = Timer.Elapsed;
            } catch (RemotingException) {
                // Ignore remoting exceptions
                // .NET Remoting will throw an exception if the host application is unreachable
        ...
        public ScreenshotRequest Request { get => _request; set => Interlocked.Exchange(ref _request, value); }
