...
namespace Capture {
    public class CaptureProcess : IDisposable {
        ...
        public CaptureProcess(Process process, CaptureConfig config, CaptureInterface captureInterface) {
            ...
            if (process.MainWindowHandle == IntPtr.Zero) throw new ProcessHasNoWindowHandleException();

            // Skip if the process is already hooked (and we want to hook multiple applications)
            if (HookManager.IsHooked(process.Id)) throw new ProcessAlreadyHookedException();
            ...
            _screenshotServer = RemoteHooking.IpcCreateServer(ref _channelName, WellKnownObjectMode.Singleton, CaptureInterface);
